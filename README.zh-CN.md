...
| 功能 | 说明 |
| --- | --- |
| **可选历史累计** | 默认关闭；需要时再开启，完整历史会在后台低优先级统计，并跨启动复用聚合缓存。 |
...
| **快速增量扫描** | 优先读取当前周期；启动时立即显示上次聚合结果，再在后台更新有变化的日志。 |
...
启动后点击 macOS 菜单栏中的统计图标即可查看用量。应用会优先展示当前周期，并保存不含原始会话内容的聚合快照，因此以后启动无需等待重新计算即可显示结果。
...
历史累计默认关闭，因为它需要扫描更广的日志范围。需要时可在 **设置** 中开启“统计历史累计”：任务会以后台优先级运行，结果跨启动缓存；除非手动选择“重新统计历史”，否则最多每天自动更新一次。设置菜单还可以开启登录时启动、设置滚动限制通知、切换虚构数据演示模式，以及手动检查 GitHub 新版本。
...
- 不包含遥测或分析 SDK。
- 不需要 OpenAI API Key。
- 刷新偏好和通知去重标记保存在 macOS 本地用户设置中；Application Support 中还会保存一个仅含聚合数字的快照，用于下次立即显示，缓存不会复制原始会话内容。
